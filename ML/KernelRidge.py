...
def KRR_global(
    X_train, Y_train, X_test, best_params, kernel="rbf", norm=2, dist1="na", dist2="na"
):
    """
...
def covariance(dist, kernel, params):
    if kernel == "linear":
        ...
        K = ((params["sigma0"] ** 2) + ((params["sigma1"] ** 2) * dist)) ** params[
            "order"
        ]
        return K
...
def GridSearchCV(
    X, Y, params, cv=4, kernel="rbf", norm=2, FCHL=False, local=False, q=None
):
    """
    ...
    if FCHL == False:
        if kernel in ["linear", "polynomial"]:
            dist = [
                (
                    kernels.linear_kernel(X_train[i], X_train[i]),
                    kernels.linear_kernel(X_train[i], X_test[i]),
                )
                for i in range(cv)
            ]
        else:
            dist = [
                (
                    p_distance(X_train[i], X_train[i], p=norm),
                    p_distance(X_train[i], X_test[i], p=norm),
                )
                for i in range(cv)
            ]
    if kernel in ["rbf", "gaussian", "Gaussian", "laplacian", "matern1", "matern2"]:
        ...
        mae = np.inf
        for i, p, j, a in product(
            params["lambda"], params["order"], params["sigma0"], params["sigma1"]
        ):
            mae_new = []
            for k in range(cv):
                y = KRR_global(
...
def KRR_local(X_train, Q_train, Y_train, X_test, Q_test, kernel, best_params):
    ...
    try:
        ...
        try:
            ...
            if kernel in ["gaussian", "rbf", "Gaussian"]:
                k = kernels.get_local_kernels(
                    X_train, X_test, Q_train, Q_test, [sigma]
                ).T
            elif kernel == "laplacian":
                k = kernels.get_local_kernel_laplacian(
                    X_train, X_test, Q_train, Q_test, [sigma]
                ).T
            elif kernel == "mbdf":
                k = kernels.get_local_kernel_mbdf(
                    X_train, X_test, Q_train, Q_test, sigma
                ).T
            elif kernel == "matern1":
                k = kernels.get_local_kernel_matern(
                    X_train, X_test, Q_train, Q_test, sigma, order=1
                ).T
            elif kernel == "matern2":
                k = kernels.get_local_kernel_matern(
                    X_train, X_test, Q_train, Q_test, sigma, order=2
                ).T
            return np.dot(k.T, alpha)
...
def GridSearchCV_local(X, Q, Y, params, kernel="Gaussian", cv=4):
    ...
    for i, j in tqdm(list(product(params["lambda"], params["length"]))):
        mae_new = []
        for k in range(cv):
            y = KRR_local(
                X_train[k],
...
def GridSearchCV_soap(
    X, Y, params, cv=4, kernel="average", metric="rbf", normalized=False
):
    kf = KFold(n_splits=cv)
    X_train, X_test = [], []
    Y_train, Y_test = [], []
...
def GridSearch_SVC(
    k_list, lam_list, Y_train, index_train, index_test, Y_test, progress=False
):
    score = 0
    from itertools import product
